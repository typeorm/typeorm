...
export class PostgresQueryRunner extends BaseQueryRunner {
    ...
    dropIndex(tableOrName, indexOrName) {
        return __awaiter(this, void 0, void 0, function* () {
            const table = tableOrName instanceof Table ? tableOrName : yield this.getCachedTable(tableOrName);
            const name = indexOrName instanceof TableIndex ? indexOrName.name : indexOrName;
            if (!name) {
                throw new Error(`Supplied TableIndex index was not found in table ${table.name}/${name}`);
            }
            const up = this.dropIndexSql(table, name);
            const down = indexOrName instanceof TableIndex ? this.createIndexSql(table, indexOrName) : [];
            yield this.executeQueries(up, down);
    ...
    dropIndexSql(table, indexOrName) {
        ...
        const schema = this.extractSchema(table);
        return schema ? new Query(`DROP INDEX IF EXISTS "${schema}"."${indexName}"`) : new Query(`DROP INDEX IF EXISTS "${indexName}"`);
    }
    /**
