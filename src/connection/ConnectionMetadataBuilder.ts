...
export class ConnectionMetadataBuilder {
    ...
    buildEntityMetadatas(
        ...
        const entityClasses: Function[] = entityClassesOrSchemas.filter(
            (entityClass) => entityClass instanceof EntitySchema === false
        ) as any;
        const entitySchemas: EntitySchema<
            any
        >[] = entityClassesOrSchemas.filter(
            (entityClass) => entityClass instanceof EntitySchema
        ) as any;
        ...
        allEntityClasses.forEach((entityClass) => {
            ...
            if (entityClass instanceof EntitySchema) {
                ...
                allEntityClasses.slice(
                    allEntityClasses.indexOf(entityClass),
                    1
                );
            }
        });
        ...
        const metadataArgsStorageFromSchema = new EntitySchemaTransformer().transform(
            entitySchemas
        );
        const schemaEntityMetadatas = new EntityMetadataBuilder(
            this.connection.namingStrategy || new DefaultNamingStrategy(),
