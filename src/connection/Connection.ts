...
export class Connection {
    ...
    createQueryBuilder<Entity>(
        ...
        if (alias) {
            const metadata = this.getMetadata(
                entityOrRunner as Function | EntitySchema<Entity> | string
            );
    ...
    protected buildMetadatas(): void {
        ...
        const allEntities = [...entityClasses, ...entityDirectories];
        const key: string = `${this.options.type}.${allEntities.join()}`;

        let metadata:
            | IConnectionMetadata
            | undefined = connectionMetadataCache.get(key);
