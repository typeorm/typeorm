...
import { GaussDBDriver } from "./GaussDBDriver"
import { BroadcasterResult } from "../../subscriber/BroadcasterResult"

/**
 * Runs queries on a single gaussdb database connection.
 */
export class GaussDBQueryRunner extends BaseQueryRunner implements QueryRunner {
    // -------------------------------------------------------------------------
    ...
    constructor(driver: GaussDBDriver, mode: ReplicationMode) {
        super()
        this.driver = driver
            ...
            this.databaseConnectionPromise = this.driver
                ...
                .then(([connection, release]: any[]) => {
                    ...
                    const onErrorCallback = (err: Error) =>
                        this.releaseGaussDBConnection(err)
     ...
     */
    private async releaseGaussDBConnection(err?: Error) {
        if (this.isReleased) {
            return
        }
    ...
    release(): Promise<void> {
        return this.releaseGaussDBConnection()
    }
     ...
     * Note: GaussDB does not support database creation inside a transaction block.
     */
    ...
    /**
     * Drops database.
     * Note: GaussDB does not support database dropping inside a transaction block.
     */
    ...
    /**
     * Loads GaussDB version.
     */
    protected async getVersion(): Promise<string> {
        const result = await this.query(`SELECT version()`)
        return result[0]["version"].replace(/^.*?Kernel ([\d.]+) .*$/, "$1")
    }
    ...
    /**
     * Checks if the GaussDB server has support for partitioned tables
     */
