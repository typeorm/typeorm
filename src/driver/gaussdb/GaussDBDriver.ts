...
import { GaussDBConnectionCredentialsOptions } from "./GaussDBConnectionCredentialsOptions"
import { GaussDBConnectionOptions } from "./GaussDBConnectionOptions"
import { GaussDBQueryRunner } from "./GaussDBQueryRunner"
...
/**
 * Organizes communication with GaussDB.
 */
export class GaussDBDriver implements Driver {
    // -------------------------------------------------------------------------
    // Public Properties
    // -------------------------------------------------------------------------
     ...
     * Connection options.
     */
    options: GaussDBConnectionOptions

    /**
     * Version of GaussDB. Requires a SQL query to the DB, so it is not always set
     */
    version?: string
    ...
    /**
     * Schema that's used internally by GaussDB for object resolution.
     *
    ...
    constructor(connection?: DataSource) {
        ...
        this.options = connection.options as GaussDBConnectionOptions
        this.isReplicated = this.options.replication ? true : false
        // if (this.options.useUTC) {
        //     process.env.PGTZ = "UTC"
        // }
        // load gaussdb package
    ...
    /**
     * Makes any action after connection (e.g. create extensions in GaussDB driver).
     */
    ...
    async disconnect(): Promise<void> {
        if (!this.master)
            return Promise.reject(new ConnectionIsNotSetError("gaussdb"))
    ...
    createQueryRunner(mode: ReplicationMode): QueryRunner {
        return new GaussDBQueryRunner(this, mode)
    }
     ...
     * GaussDB sorts json values before it is saved, so in that case a deep comparison has to be performed to see if has changed.
     */
    ...
    protected loadDependencies(): void {
        try {
            const gaussdb = this.options.driver || PlatformTools.load("pg")
            this.gaussdb = gaussdb
            try {
                const pgNative =
                    this.options.nativeDriver || PlatformTools.load("pg-native")
                if (pgNative && this.gaussdb.native)
                    this.gaussdb = this.gaussdb.native
            } catch (e) {}
        } catch (e) {
            // todo: better error for browser env
            throw new DriverPackageNotInstalledError("GaussDB", "pg")
        }
    }
    ...
    protected async createPool(
        options: GaussDBConnectionOptions,
        credentials: GaussDBConnectionCredentialsOptions,
    ): Promise<any> {
        const { logger } = this.connection
        ...
        if (options.parseInt8 !== undefined) {
            if (
                this.gaussdb.defaults &&
                Object.getOwnPropertyDescriptor(
                    this.gaussdb.defaults,
                    "parseInt8",
                )?.set
            ) {
                this.gaussdb.defaults.parseInt8 = options.parseInt8
            } else {
                logger.log(
                    "warn",
                    "Attempted to set parseInt8 option, but the gaussdb driver does not support setting defaults.parseInt8. This option will be ignored.",
                )
            }
        }

        // create a connection pool
        const pool = new this.gaussdb.Pool(connectionOptions)

        const poolErrorHandler =
            options.poolErrorHandler ||
            ((error: any) =>
                logger.log("warn", `GaussDB pool raised an error. ${error}`))
