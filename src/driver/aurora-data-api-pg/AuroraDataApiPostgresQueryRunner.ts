 ...
 */
export class AuroraDataApiPostgresQueryRunner extends BaseQueryRunner implements QueryRunner {
    ...
    async clearDatabase(): Promise<void> {
        ...
        schemas.push(this.driver.options.database || "current_schema()");
    ...
    protected async loadViews(viewNames: string[]): Promise<View[]> {
        ...
        const viewsCondition = viewNames.map(viewName => {
            ...
            if (!name) {
                name = schema;
                schema = this.driver.options.database || currentSchema;
            }
        ...
        return dbViews.map((dbView: any) => {
            const view = new View();
            const schema = dbView["schema"] === currentSchema && !this.driver.options.database ? undefined : dbView["schema"];
    ...
    protected async loadTables(tableNames: string[]): Promise<Table[]> {
        ...
        return Promise.all(dbTables.map(async dbTable => {
            ...
            const schema = dbTable["table_schema"] === currentSchema && !this.driver.options.database ? undefined : dbTable["table_schema"];
    ...
    protected async insertViewDefinitionSql(view: View): Promise<Query> {
        ...
        let schema = this.driver.options.database || currentSchema;
        let name = view.name;
    ...
    protected extractSchema(target: Table|string): string|undefined {
        const tableName = target instanceof Table ? target.name : target;
        return tableName.indexOf(".") === -1 ? this.driver.options.database : tableName.split(".")[0];
    }
    ...
    protected buildEnumName(table: Table, columnOrName: TableColumn|string, withSchema: boolean = true, disableEscape?: boolean, toOld?: boolean): string {
        ...
        const columnName = columnOrName instanceof TableColumn ? columnOrName.name : columnOrName;
        const schema = table.name.indexOf(".") === -1 ? this.driver.options.database : table.name.split(".")[0];
    ...
    protected escapePath(target: Table|View|string, disableEscape?: boolean): string {
        ...
        tableName = tableName.indexOf(".") === -1 && this.driver.options.database ? `${this.driver.options.database}.${tableName}` : tableName;
    ...
    protected parseTableName(target: Table|string) {
        ...
        if (tableName.indexOf(".") === -1) {
            return {
                schema: this.driver.options.database ? `'${this.driver.options.database}'` : "current_schema()",
                tableName: `'${tableName}'`
            };
        } else {
            return {
