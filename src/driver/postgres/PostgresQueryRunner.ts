...
export class PostgresQueryRunner extends BaseQueryRunner implements QueryRunner {
    ...
    async renameTable(oldTableOrName: Table|string, newTableName: string): Promise<void> {
        ...
        newTable.columns.map(col => {
            if (col.isGenerated && col.generationStrategy === "increment" && !this.driver.supportsIdentityColumn()) { // POSTGRES <10.0 SERIAL Behavior
    ...
    async changeColumn(tableOrName: Table|string, oldTableColumnOrName: TableColumn|string, newColumn: TableColumn): Promise<void> {
        ...
        if (oldColumn.type !== newColumn.type || oldColumn.length !== newColumn.length || newColumn.isArray !== oldColumn.isArray) {
            ...
            if (oldColumn.name !== newColumn.name) {
                ...
                if (oldColumn.isGenerated === true && newColumn.generationStrategy === "increment" && !this.driver.supportsIdentityColumn()) {
            ...
            if (oldColumn.isGenerated !== newColumn.isGenerated && newColumn.generationStrategy !== "uuid" && !this.driver.supportsIdentityColumn()) {
    ...
    protected buildCreateColumnSql(table: Table, column: TableColumn) {
        ...
        if (column.isGenerated === true && column.generationStrategy !== "uuid") {
            if (this.driver.supportsIdentityColumn()) { // Postgres 10+ Identity generated column
