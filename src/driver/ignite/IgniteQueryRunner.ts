...
export class IgniteQueryRunner extends BaseQueryRunner implements QueryRunner {
  ...
  async query(query: string, parameters = []) {
    ...
    const sqlQuery = new this.driver.SqlFieldsQuery(query).setArgs(...parameters).setIncludeFieldNames(true);
  ...
  async loadTables(tableNames: string[]): Promise<Table[]> {
    ...
    const tableConditions = tableNames.map((tableName) => `TABLE_NAME = '${tableName}'`).join(" OR ");
    const tableSqls = new this.driver.SqlFieldsQuery(`SELECT * FROM TABLES WHERE ${tableConditions}`)
      .setSchema("SYS")
      .setIncludeFieldNames(true);
    const columnSql = new this.driver.SqlFieldsQuery(`SELECT * FROM TABLE_COLUMNS WHERE ${tableConditions}`)
      .setSchema("SYS")
      .setIncludeFieldNames(true);
    const indicesSql = new this.driver.SqlFieldsQuery(`SELECT * FROM INDEXES WHERE ${tableConditions}`)
      .setSchema("SYS")
      .setIncludeFieldNames(true);
    ...
    const [dbTables, dbColumns, dbIndices]: [
      ...
      tableCursor.getAll().then((values: any) => this.fromPairs(tableCursor.getFieldNames(), values)),
      columnsCursor.getAll().then((values: any) => this.fromPairs(columnsCursor.getFieldNames(), values)),
      indexCursor.getAll().then((values: any) => this.fromPairs(indexCursor.getFieldNames(), values)),
    ]);
      ...
      dbTables.map(async (dbTable) => {
        ...
        const tableIndexOptions: TableIndexOptions[] = dbIndices
          ...
          .map((dbIndex) => ({
            ...
            columnNames: (dbIndex.COLUMNS || "").split(",").map((s) => s.trim().split(" ")[0]),
          }));
  ...
  async hasTable(tableOrName: Table | string): Promise<boolean> {
    ...
    const query = new this.driver.SqlFieldsQuery(`SELECT * FROM TABLES WHERE TABLE_NAME = '${name}'`).setSchema("SYS");
  ...
  async createTable(table: Table, ifNotExist = false, createForeignKeys = true, createIndexes = true): Promise<void> {
    if (ifNotExist) {
  ...
  async hasColumn(tableOrName: Table | string, column: TableColumn | string): Promise<boolean> {
    ...
    const sql = this.driver.SqlFieldsQuery(`SELECT * FROM "${tableName}" WHERE COLUMN_NAME = '${columnName}'`);
  ...
  async createExclusionConstraint() {
    throw new Error("Create Exclusion Constraint is not supported by Ignite");
  }

  async createExclusionConstraints() {
    throw new Error("Create Exclusion Constraints is not supported by Ignite");
  }
  ...
  async dropExclusionConstraints() {
    throw new Error("Drop Exclusion Constraints is not supported by Ignite");
  }
  ...
  async clearDatabase() {
    throw new Error("Clear Database is not supported by Ignite");
  }
  ...
  protected dropTableSql(tableOrName: Table | string, ifExist = false): Query {
    const tableName = tableOrName instanceof Table ? tableOrName.name : tableOrName;
    const sql = ifExist ? `DROP TABLE IF EXISTS "${tableName}"` : `DROP TABLE "${tableName}"`;
    return new Query(sql);
  }
  ...
  protected addColumnsSql(table: Table, columns: TableColumn[]): Query {
    const columnNames = columns.map((column) => `"${column.name}" ${column.type}`).join(", ");
