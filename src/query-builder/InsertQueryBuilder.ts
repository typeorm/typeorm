...
> extends QueryBuilder<Entity> {
    ...
    async execute(): Promise<InsertResult> {
        ...
        if (this.expressionMap.insertFromSelect) {
            return this.executeInsertFromSelect()
        }
    ...
    /**
     * Executes INSERT INTO ... SELECT FROM query.
     * This is a simplified execution path that doesn't support entity listeners
     * since there are no entity instances available.
     */
    protected async executeInsertFromSelect(): Promise<InsertResult> {
        const queryRunner = this.obtainQueryRunner()
        let transactionStartedByUs: boolean = false
     ...
     *
     * @param queryBuilder - A SelectQueryBuilder instance to use as the source of values
     * @returns this query builder instance for chaining
     *
     * @example
     ...
     * This creates an INSERT INTO ... SELECT FROM statement.
     *
     * @param subQueryFactory - A callback that receives a new SelectQueryBuilder and returns it configured
     * @returns this query builder instance for chaining
     *
     * @example
     ...
     *     .into(ArchiveUser, ["name", "email"])
     *     .valuesFromSelect((qb) =>
     *         qb.select(["user.name", "user.email"])
     *           .from(User, "user")
     *           .where("user.isArchived = :archived", { archived: true })
     *     )
     *     .execute()
    ...
    valuesFromSelect(
        ...
        // Store the SelectQueryBuilder reference
    ...
    protected createInsertExpression() {
        // Handle INSERT INTO ... SELECT FROM case
        if (this.expressionMap.insertFromSelect) {
            return this.createInsertFromSelectExpression()
        }
    ...
    /**
     * Creates INSERT INTO ... SELECT FROM expression.
     */
    protected createInsertFromSelectExpression(): string {
        const tableName = this.getTableName(this.getMainTableName())

        // Build the INSERT INTO part
        let query = "INSERT INTO " + tableName

        // Add column names if specified
        if (this.expressionMap.insertColumns.length > 0) {
            const columnsExpression = this.expressionMap.insertColumns
                .map((columnName) => this.escape(columnName))
                .join(", ")
            query += `(${columnsExpression})`
        } else if (this.expressionMap.mainAlias!.hasMetadata) {
            // Use metadata columns if available and no explicit columns specified
        ...
        // Add the SELECT query (without wrapping in parentheses - standard SQL syntax)
