...
export class ReturningResultsEntityUpdator {
    ...
    async insert(
        ...
        if (
            ...
            const orderPrimaryConditions = entityIds
                .map((entityId, orderIdx) => {
                    const conditions = Object.keys(entityId)
                        .map((key) => {
                            const parameterName = `order_${metadata.targetName}_${key}_${orderIdx}`
                            parameters[parameterName] = entityId[key]

                            return `${metadata.targetName}.${key} = :${parameterName}`
                        })
                        .join(" AND ")

                    return `WHEN (${conditions}) THEN ${orderIdx}`
                })
                .join(" ")
