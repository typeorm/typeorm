...
export class ReturningResultsEntityUpdator {
    ...
    async insert(
        ...
        const generatedMaps = entities.map((entity, entityIndex) => {
            ...
            if (
                this.queryRunner.connection.driver.isUUIDGenerationSupported() ===
                false
            ) {
                metadata.generatedColumns.forEach((generatedColumn) => {
                    if (generatedColumn.generationStrategy === "uuid") {
                        ...
                        OrmUtils.mergeDeep(
                            generatedMap,
                            generatedColumn.createValueMap(uuid)
                        );
                    }
                });
            }
            ...
            this.queryRunner.manager.merge(
                ...
                entity,
                generatedMap
            ); // todo: this should not be here, but problem with below line
            return generatedMap;
        });
        ...
        entities.forEach((entity, entityIndex) => {
            ...
            this.queryRunner.manager.merge(
                this.expressionMap.mainAlias!.metadata.target as any,
                entity,
                generatedMaps[entityIndex],
                generatedMaps[entityIndex]
            ); // todo: why twice?!
        ...
        return this.expressionMap.mainAlias!.metadata.columns.filter(
            (column) => {
                return (
                    ...
                    column.calculatedDefault !== undefined ||
