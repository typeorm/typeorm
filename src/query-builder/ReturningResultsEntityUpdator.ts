...
type ReloadCriteriaResult =
    | {
          criteria: ObjectLiteral
          columns: ColumnMetadata[]
          type: "entity-id" | "conflict"
      }
    | { reason: string }
...
export class ReturningResultsEntityUpdator {
    ...
    async insert(
        ...
        if (shouldReload && !isInsertReturningSqlSupported) {
            ...
            const entityIds = reloadCriteria.map(
                (criteriaResult) => criteriaResult.criteria,
            )
            const reloadColumns = this.getUniqueColumns([
                ...metadata.primaryColumns,
                ...insertionColumns,
                ...reloadCriteria.flatMap(
                    (criteriaResult) => criteriaResult.columns,
                ),
            ])
            ...
            const selectQueryBuilder = this.queryRunner.manager
                .createQueryBuilder()
                .select(
                    reloadColumns.map(
                        (column) =>
            ...
            if (
                reloadCriteria.every(
                    (criteriaResult) => criteriaResult.type === "entity-id",
                )
            ) {
                selectQueryBuilder.whereInIds(entityIds)
            } else {
                selectQueryBuilder.where(entityIds)
            }
    ...
    protected getReloadCriteriaMap(
        entity: ObjectLiteral,
        columns: ColumnMetadata[],
    ): ObjectLiteral {
        return columns.reduce((criteria, column) => {
            OrmUtils.mergeDeep(
                criteria,
                column.createValueMap(column.getEntityValue(entity)),
            )
            return criteria
        }, {} as ObjectLiteral)
    }
    ...
    protected getUniqueColumns(columns: ColumnMetadata[]): ColumnMetadata[] {
        return columns.filter(
            (column, index) =>
                columns.findIndex(
                    (candidate) =>
                        candidate.propertyPath === column.propertyPath,
                ) === index,
        )
    }
