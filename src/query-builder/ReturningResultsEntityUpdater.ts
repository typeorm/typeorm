...
export class ReturningResultsEntityUpdater {
    ...
    async update(updateResult: UpdateResult, entities: ObjectLiteral[]): Promise<void> {
        ...
        await Promise.all(entities.map(async (entity, entityIndex) => {
            ...
            if (this.queryRunner.connection.driver.config.returningClause) {
    ...
    async insert(insertResult: InsertResult, entities: ObjectLiteral[]): Promise<void> {
        ...
        const generatedMaps = entities.map((entity, entityIndex) => {
            ...
            if (!this.queryRunner.connection.driver.config.uuidGeneration) {
                metadata.generatedColumns.forEach(generatedColumn => {
                    if (generatedColumn.generationStrategy === "uuid") {
                        ...
                        if (!uuid) uuid = this.expressionMap.generatedUuids[entityIndex]?.[generatedColumn.databaseName];
        ...
        if (!this.queryRunner.connection.driver.config.returningClause && insertionColumns.length > 0) {
    ...
    getInsertionReturningColumns(): ColumnMetadata[] {
        ...
        const needToCheckGenerated = this.queryRunner.connection.driver.config.returningClause !== undefined;
