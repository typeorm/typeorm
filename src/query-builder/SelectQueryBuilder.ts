    ...
    implements WhereExpressionBuilder
{
     ...
     */
    async getCount(distinctColumns?: string[]): Promise<number> {
        if (this.expressionMap.lockMode === "optimistic")
            throw new OptimisticLockCanNotBeUsedError()
        ...
        try {
            ...
            this.expressionMap.queryEntity = false
            const results = await this.executeCountQuery(
                queryRunner,
                distinctColumns,
            )
    ...
    private computeCountExpression(distinctColumns?: string[]): string {
        const mainAlias = this.expressionMap.mainAlias!.name // todo: will this work with "fromTableName"?
        const metadata = this.expressionMap.mainAlias!.metadata

        const countColumns = this.getCountColumns(metadata, distinctColumns)
    ...
    private getCountColumns(
        metadata: EntityMetadata,
        distinctColumns?: string[],
    ): ColumnMetadata[] {
        if (!distinctColumns || distinctColumns.length === 0) {
            return []
        }
        ...
        for (const col of distinctColumns) {
    ...
    protected async executeCountQuery(
        ...
        const countSql = this.computeCountExpression(distinctColumns)
