    ...
    implements WhereExpressionBuilder
{
    ...
    protected createOrderByExpression() {
        ...
        return (
            ` ORDER BY ${this.createOrderByNullsSQLServer(orderBys)}` +
    ...
    private createOrderByNullsSQLServer(orderBys: any) {
        let sqlServerOrder = undefined
        if (this.connection.driver.options.type === "mssql")
            sqlServerOrder = Object.keys(orderBys)
                .filter(
                    (columnName) => typeof orderBys[columnName] !== "string",
                )
                .map((columnName) => {
                    const last =
                        (orderBys[columnName] as any).nulls === "NULLS LAST"
                    return (
                        `CASE WHEN ` +
                        columnName +
                        ` IS NULL THEN ${last ? "1" : "0"} ELSE ${
                            last ? "0" : "1"
                        } END`
                    )
                })
        ...
        return sqlServerOrder ? sqlServerOrder + ", " : ""
    }
    ...
    protected async executeEntitiesAndRawResults(
        ...
        if (
            ...
            rawResults = await new SelectQueryBuilder(
                ...
                .select(
                    `DISTINCT ${this.createOrderByNullsSQLServer(
                        orderBys,
                    )}${querySelects.join(", ")}`,
                )
                .addSelect(selects)
