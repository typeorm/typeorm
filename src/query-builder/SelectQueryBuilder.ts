    ...
    implements WhereExpressionBuilder
{
     ...
     */
    async getRawMany<T = any>(timestamp?: Date): Promise<T[]> {
        ...
        try {
            ...
            const results = await this.loadRawResults(queryRunner, timestamp)
     ...
     * @param {Date} [timestamp] A timestamp to get datasets at a specific time.
     */
    async getOne(timestamp?: Date): Promise<Entity | null> {
        const results = await this.getRawAndEntities(timestamp)
     ...
     * @param {Date} [timestamp] A timestamp to get datasets at a specific time.
     */
    async getMany(timestamp?: Date): Promise<Entity[]> {
        if (this.expressionMap.lockMode === "optimistic")
            throw new OptimisticLockCanNotBeUsedError()
        ...
        if (
            ...
            if (rawResults.length > 0) {
                ...
                rawResults = await this.clone()
                    ...
                    .loadRawResults(queryRunner, timestamp)
            }
        } else {
            rawResults = await this.loadRawResults(queryRunner, timestamp)
        }
     ...
     * @param {Date} [timestamp] A timestamp to get datasets at a specific time. If undefined use a date in the future to get the current datasets.
     */
