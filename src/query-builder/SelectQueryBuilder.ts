...
export class SelectQueryBuilder<Entity> extends QueryBuilder<Entity> implements WhereExpression {
    ...
    async getRawMany<T = any>(): Promise<T[]> {
        ...
        try {
            ...
            const results = await this.loadRawResults(this.queryRunner);
    ...
    async getCount(): Promise<number> {
        ...
        try {
            ...
            this.expressionMap.queryEntity = false;
            const results = await this.executeCountQuery(this.queryRunner);
    ...
    async getManyAndCount(): Promise<[Entity[], number]> {
        ...
        try {
            ...
            this.expressionMap.queryEntity = false;
            const count = await this.executeCountQuery(this.queryRunner);
    ...
    async stream(): Promise<ReadStream> {
        ...
        try {
            ...
            const results = this.queryRunner.stream(sql, parameters);
     ...
     * Executes sql generated by query builder and returns object with raw results and entities created from them.
     */
    protected async executeEntitiesAndRawResults(): Promise<{ entities: Entity[], raw: any[] }> {
        ...
        if ((this.expressionMap.skip || this.expressionMap.take) && this.expressionMap.joinAttributes.length > 0) {
            ...
            rawResults = await this.createQueryBuilder()
            ...
            if (rawResults.length > 0) {
                ...
                rawResults = await this.clone()
                    ...
                    .loadRawResults(this.queryRunner);
            }

        } else {
            rawResults = await this.loadRawResults(this.queryRunner);
        }
