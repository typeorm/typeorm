    ...
    protected transformRawResultsGroup(
        ...
        if (metadata.discriminatorColumn) {
            ...
            const discriminatorMetadata = metadata.childEntityMetadatas.find(
                (childEntityMetadata) => {
                    return !!discriminatorValues.find(
                        (value) =>
        ...
        this.rawRelationIdResults.forEach((rawRelationIdResult) => {
            ...
            const idMaps = rawRelationIdResult.results
                .map((result) => {
                    ...
                    if (
                        OrmUtils.compareIds(entityPrimaryIds, valueMap) ===
                        false
                    )
                        return;
                    ...
                    const idMap = columns.reduce((idMap, column) => {
                        ...
                        if (
                            ...
                            if (column.referencedColumn)
                                ...
                                value = column.referencedColumn.createValueMap(
                                    value
                                );
        ...
        this.rawRelationCountResults
            ...
            .forEach((rawRelationCountResult) => {
                ...
                if (relation.isOneToMany) {
                    referenceColumnName = relation.inverseRelation!
                        .joinColumns[0].referencedColumn!.databaseName; // todo: fix joinColumns[0]
                } else {
    ...
    private extractEntityPrimaryIds(
        ...
        return columns.reduce((data, column) => {
            data[column.databaseName] =
                relationIdRawResult[column.databaseName];
            return data;
        }, {} as ObjectLiteral);
    }
