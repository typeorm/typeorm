...
export class EntityMetadataBuilder {
    ...
    build(entityClasses?: Function[]): EntityMetadata[] {
        ...
        this.mergeWithUserDefinedJunctions(entityMetadatas)
    ...
    /**
     * Reconciles automatically generated junction metadata with user-defined entities
     * that target the same join table. We prefer the user entity so that additional
     * columns and custom configuration are preserved when relations operate on the
     * join table.
     */
    protected mergeWithUserDefinedJunctions(
        ...
        const junctionsToRemove: EntityMetadata[] = []

        for (const junctionMetadata of entityMetadatas.filter(
            (metadata) => metadata.isJunction,
        )) {
            const userDefinedMetadata = userMetadatasByTable.get(
                junctionMetadata.tablePath,
            )
            if (!userDefinedMetadata) continue

            const mapColumn = (column: ColumnMetadata): ColumnMetadata => {
                const match = userDefinedMetadata.columns.find(
                    (candidate) =>
                        candidate.databaseName === column.databaseName,
                )
                if (!match) {
                    throw new TypeORMError(
                        `Column "${column.databaseName}" referenced by a many-to-many join table was not found in the user-defined entity "${userDefinedMetadata.name}".`,
                    )
                }
                return match
            }
            ...
            const clonedForeignKeys = junctionMetadata.foreignKeys.map(
                (foreignKey) => {
                    ...
                    const cloned = new ForeignKeyMetadata({
                        entityMetadata: userDefinedMetadata,
            ...
            const clonedIndices = junctionMetadata.ownIndices.map((index) => {
                return new IndexMetadata({
                    entityMetadata: userDefinedMetadata,
                    columns: index.columns.map(mapColumn),
                    args: {
                        target: userDefinedMetadata.target!,
                        name: index.givenName,
                        unique: index.isUnique,
                        spatial: index.isSpatial,
                        fulltext: index.isFulltext,
                        nullFiltered: index.isNullFiltered,
                        parser: index.parser,
                        where: index.where,
                        synchronize: index.synchronize,
                        sparse: index.isSparse,
                        background: index.isBackground,
                        concurrent: index.isConcurrent,
            ...
            const existingForeignKeySignatures = new Set(
                userDefinedMetadata.foreignKeys.map((foreignKey) => {
            ...
            const existingIndexSignatures = new Set(
                userDefinedMetadata.indices.map((index) =>
                    index.columns
                        .map((column) => column.databaseName)
                        .sort()
                        .join(","),
            ...
            const indicesToRegister = clonedIndices.filter((index) => {
                const signature = index.columns
                    .map((column) => column.databaseName)
                    .sort()
                    .join(",")
                if (existingIndexSignatures.has(signature)) return false
                existingIndexSignatures.add(signature)
                return true
            })

            userDefinedMetadata.tableMetadataArgs.type = "junction"
            userDefinedMetadata.tableType = "junction"
            userDefinedMetadata.isJunction = true
            userDefinedMetadata.ownerColumns = ownerColumns
            userDefinedMetadata.inverseColumns = inverseColumns
            userDefinedMetadata.foreignKeys.push(...foreignKeysToRegister)
            userDefinedMetadata.ownIndices.push(...indicesToRegister)
            ...
            this.computeEntityMetadataStep2(userDefinedMetadata)
            this.computeInverseProperties(userDefinedMetadata, entityMetadatas)

            junctionsToRemove.push(junctionMetadata)
        }

        if (junctionsToRemove.length > 0) {
            for (const metadata of junctionsToRemove) {
                const index = entityMetadatas.indexOf(metadata)
                if (index !== -1) {
                    entityMetadatas.splice(index, 1)
                }
            }
        }
    }
