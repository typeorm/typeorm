...
export class DbQueryResultCache implements QueryResultCache {
            ...
            (queryRunner) => {
                ...
                if (options.identifier) {
                    ...
                    if (this.connection.driver.options.type === "oracle") {
                        return qb
                            .where(
                                `dbms_lob.compare(${qb.escape(
                                    "cache",
                                )}.${qb.escape("query")}, :query) = 0`,
                                { query: options.query },
                            )
                            .cache(false) // disable cache to avoid infinite loops when cache is alwaysEnable
                            .getRawOne()
    ...
    async storeInCache(
        ...
        const existingQueryRunner =
            queryRunner?.getReplicationMode() === "master"
                ? queryRunner
                : undefined
        return this.connection.runWithQueryRunner(
            existingQueryRunner,
    ...
    async clear(queryRunner: QueryRunner): Promise<void> {
        return this.connection.runWithQueryRunner(queryRunner, (queryRunner) =>
            queryRunner.clearTable(this.queryResultCacheTable),
        )
    }
