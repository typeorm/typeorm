...
export class MongoEntityManager extends EntityManager {
    ...
    async find<Entity>(entityClassOrName: EntityTarget<Entity>, optionsOrConditions?: FindManyOptions<Entity> | Partial<Entity>): Promise<Entity[]> {
        ...
        const cursor = await this.createEntityCursor(entityClassOrName, query as Filter<Entity>);
    ...
    async findByIds<Entity>(entityClassOrName: EntityTarget<Entity>, ids: any[], optionsOrConditions?: FindManyOptions<Entity> | Partial<Entity>): Promise<Entity[]> {
        ...
        const cursor = this.createEntityCursor(entityClassOrName, query as Filter<Entity>);
        if (FindOptionsUtils.isFindManyOptions(optionsOrConditions)) {
            if (optionsOrConditions.select)
                cursor.project(this.convertFindOptionsSelectToProjectCriteria(optionsOrConditions.select));
            if (optionsOrConditions.skip)
    ...
    async update<Entity>(target: EntityTarget<Entity>, criteria: string | string[] | number | number[] | Date | Date[] | ObjectId | ObjectId[] | FindConditions<Entity>, partialEntity: QueryDeepPartialEntity<Entity>): Promise<UpdateResult> {
        const mongoResult = await this.updateMany(target, this.convertMixedCriteria(this.connection.getMetadata(target), criteria), { $set: partialEntity });
        return { raw: mongoResult, affected: mongoResult.modifiedCount } as UpdateResult;
    }
    ...
    async delete<Entity>(target: EntityTarget<Entity>, criteria: string | string[] | number | number[] | Date | Date[] | ObjectId | ObjectId[] | FindConditions<Entity>): Promise<DeleteResult> {
        ...
        return { raw: mongoResult, affected: mongoResult.deletedCount } as DeleteResult;
    }
     ...
     */
    createCursor<Entity, T = any>(entityClassOrName: EntityTarget<Entity>, query: Filter<Entity> = {}): FindCursor<T> {
        const metadata = this.connection.getMetadata(entityClassOrName);
     ...
     */
    createEntityCursor<Entity>(entityClassOrName: EntityTarget<Entity>, query: Filter<Entity> = {}): FindCursor<Entity> {
        const metadata = this.connection.getMetadata(entityClassOrName);
     ...
     */
    findOneAndDelete<Entity>(entityClassOrName: EntityTarget<Entity>, query: ObjectLiteral, options?: FindOneAndDeleteOptions): Promise<ModifyResult<Document>> {
        const metadata = this.connection.getMetadata(entityClassOrName);
     ...
     */
    findOneAndReplace<Entity>(entityClassOrName: EntityTarget<Entity>, query: Filter<Document>, replacement: Document, options?: FindOneAndReplaceOptions): Promise<ModifyResult<Document>> {
        const metadata = this.connection.getMetadata(entityClassOrName);
     ...
     */
    findOneAndUpdate<Entity>(entityClassOrName: EntityTarget<Entity>, query: Filter<Document>, update: UpdateFilter<Document>, options?: FindOneAndUpdateOptions): Promise<ModifyResult<Document>> {
        const metadata = this.connection.getMetadata(entityClassOrName);
     ...
     */
    mapReduce<Entity>(entityClassOrName: EntityTarget<Entity>, map: MapFunction, reduce: ReduceFunction | string, options?: MapReduceOptions): Promise<Document | Document[]> {
        const metadata = this.connection.getMetadata(entityClassOrName);
     ...
     */
    protected applyEntityTransformationToCursor<Entity>(metadata: EntityMetadata, cursor: FindCursor<Entity> | AggregationCursor<Entity>) {
        const ParentCursor = new FindCursor();
        const queryRunner = this.mongoQueryRunner;
        cursor.toArray = function (callback?: MongoCallback<Entity[]>) {
            if (callback) {
                ParentCursor.toArray.call(this, (error: MongoError, results: Entity[]): void => {
                    if (error) {
                ...
                return ParentCursor.toArray.call(this).then((results: Entity[]) => {
                    const transformer = new DocumentToEntityTransformer();
                    const entities = transformer.transformAll(results, metadata);
        ...
        cursor.next = function (callback?: MongoCallback<CursorResult>) {
            if (callback) {
                ParentCursor.next.call(this, (error: MongoError, result: CursorResult): void => {
                    if (error || !result) {
                ...
                return ParentCursor.next.call(this).then((result: Entity) => {
