...
export class EntityManager {
    ...
    async transaction<T>(
        ...
        const runInTransaction =
            typeof isolationOrRunInTransaction === 'function' ? isolationOrRunInTransaction : runInTransactionParam
     ...
     */
    query(query: string | { text: string; values?: any[] }, values?: any[]): Promise<any> {
        return this.connection.query(query, values, this.queryRunner)
    }
    ...
    async preload<Entity>(
        ...
        if (transformedEntity) return this.merge(entityClass as any, transformedEntity as Entity, entityLike)
    ...
    remove<Entity>(
        ...
        // normalize mixed parameters
        const target =
            arguments.length > 1 && (targetOrEntity instanceof Function || typeof targetOrEntity === 'string')
                ? (targetOrEntity as Function | string)
                : undefined
    ...
    async count<Entity>(
        entityClass: EntityTarget<Entity>,
        optionsOrConditions?: FindConditions<Entity> | FindOneOptions<Entity> | FindManyOptions<Entity>
    ): Promise<number> {
        const metadata = this.connection.getMetadata(entityClass)
    ...
    getRepository<Entity>(target: EntityTarget<Entity>): Repository<Entity> {
        ...
        if (!this.connection.hasMetadata(target)) throw new RepositoryNotFoundError(this.connection.name, target)
    ...
    getCustomRepository<T>(customRepository: ObjectType<T>): T {
        const entityRepositoryMetadataArgs = getMetadataArgsStorage().entityRepositories.find((repository) => {
            return (
                repository.target ===
                (customRepository instanceof Function ? customRepository : (customRepository as any).constructor)
            )
        })
        ...
        if (entityRepositoryInstance instanceof AbstractRepository) {
            if (!(entityRepositoryInstance as any)['manager']) (entityRepositoryInstance as any)['manager'] = this
        }
