...
describe.only("github issues > #8747 QueryBuilder update handles Date objects wrong on a ManyToOne relation ship.", () => {
    ...
    it("should correctly update the datetime field", () => Promise.all(connections.map(async connection => {
      ...
      const carReloaded = await Car.findOne({ where: { uuid: car.uuid } })

      expect(carReloaded).to.exist
      expect(carReloaded!.latestRecordTimestamp?.getTime()).to.be.equal(record.timestamp?.getTime())
    })));
});
