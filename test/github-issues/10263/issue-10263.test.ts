...
describe("github issues > #10263 Postgres @Tree specify schema for closure tables", () => {
        ...
        Promise.all(
            dataSources.map(async (connection) => {
                ...
                const table = (await queryRunner.getTable(
                    "test_schema.category_xyz_closure",
                ))!
    ...
    it("should have correctly named ancestor and descendant columns", () =>
        Promise.all(
            dataSources.map(async (connection) => {
                const queryRunner = connection.createQueryRunner()
                const table = await queryRunner.getTable(
                ...
                const ancestorColumn = table?.columns.find(
                    (column) => column.name === "ancestor_xyz_id",
                )
                const descendantColumn = table?.columns.find(
                    (column) => column.name === "descendant_xyz_id",
                )
