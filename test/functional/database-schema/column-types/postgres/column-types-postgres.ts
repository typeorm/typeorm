...
describe("database schema > column types > postgres", () => {
    ...
    describe("all types should work correctly", () => {
        it("persist and hydrate", () =>
            Promise.all(
                connections.map(async (connection) => {
                    ...
                    loadedPost.doublePrecision.should.be.equal(
                        post.doublePrecision,
                    )
                    ...
                    loadedPost.simpleArray[0].should.be.equal(
                        post.simpleArray[0],
                    )
                    loadedPost.simpleArray[1].should.be.equal(
                        post.simpleArray[1],
                    )
                    loadedPost.simpleArray[2].should.be.equal(
                        post.simpleArray[2],
                    )
                    ...
                    table!
                        ...
                        .type.should.be.equal("integer")
                    table!
                        .findColumnByName("int4")!
                        .type.should.be.equal("integer")
                    table!
                        .findColumnByName("int")!
                        .type.should.be.equal("integer")
                    table!
                    ...
                    table!
                        ...
                        .type.should.be.equal("double precision")
                    table!
                        .findColumnByName("real")!
                        .type.should.be.equal("real")
                    table!
                        .findColumnByName("float4")!
                        .type.should.be.equal("real")
                    table!
                        .findColumnByName("money")!
                        .type.should.be.equal("money")
                    table!
                    ...
                    table!
                        .findColumnByName("bytea")!
                        .type.should.be.equal("bytea")
                    table!
                    ...
                    table!
                        ...
                        .type.should.be.equal("point")
                    table!
                        .findColumnByName("line")!
                        .type.should.be.equal("line")
                    table!
                        .findColumnByName("lseg")!
                        .type.should.be.equal("lseg")
                    table!.findColumnByName("box")!.type.should.be.equal("box")
                    table!
                        .findColumnByName("path")!
                        .type.should.be.equal("path")
                    table!
                    ...
                    table!
                        .findColumnByName("cidr")!
                        .type.should.be.equal("cidr")
                    table!
                        .findColumnByName("inet")!
                        .type.should.be.equal("inet")
                    table!
                    ...
                    table!
                        .findColumnByName("jsonb")!
                        .type.should.be.equal("jsonb")
                    table!
        ...
        it("persist and hydrate when options are specified on columns", () =>
            Promise.all(
                connections.map(async (connection) => {
                    const postRepository =
                        connection.getRepository(PostWithOptions)
                    const queryRunner = connection.createQueryRunner()
                    ...
                    loadedPost.time
                        .valueOf()
                        .should.be.equal(post.time.valueOf())
        ...
        it("persist and hydrate when types are not specified on columns", () =>
            Promise.all(
                connections.map(async (connection) => {
                    const postRepository =
        ...
        validJsonPaths.forEach(([path, canonical]) => {
            it(`should insert and retrieve jsonpath values as strings for: ${path}`, () =>
                Promise.all(
                    connections.map(async (connection) => {
                        const repository =
                            connection.getRepository(JsonPathExample)
