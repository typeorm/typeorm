...
describe("json > json-operator", () => {
    ...
    it("should find the entities matching the json operator with one to many relations", () =>
        Promise.all(
            connections.map(async (connection) => {
                const author1 = new Author()
                author1.name = { fr: 'lejeune', en: 'theyoung' }
                ...
                const [authorSaved1] = await connection.manager.save([author1, author2])

                const post1 = new Post()
                post1.title = "Post #1"
                post1.category = { fr: "Livres" , en: "Books" }
                post1.author = authorSaved1;
