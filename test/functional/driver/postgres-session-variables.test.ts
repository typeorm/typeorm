...
describe("driver > session variables validation", () => {
    it("should throw on an invalid session variable name", () => {
        expect(() =>
            PostgresUtils.buildSessionVariablesHandler({ "bad name!": "x" }),
        ).to.throw(/Invalid session variable name/)
    })

    it("should throw on a null or undefined value", () => {
        expect(() =>
            PostgresUtils.buildSessionVariablesHandler({
                statement_timeout: undefined,
            }),
        ).to.throw(/must not be null or undefined/)
    })

    it("should accept a namespaced (dotted) variable name", () => {
        expect(() =>
            PostgresUtils.buildSessionVariablesHandler({
                "my_app.setting": "x",
            }),
    ...
    it("should return undefined when nothing is configured", () => {
        expect(PostgresUtils.buildSessionVariablesHandler(undefined)).to.be
            .undefined
    })
})

describe("postgres driver > session variables", () => {
    ...
    before(async () => {
        dataSources = await createTestingConnections({
            ...
            driverSpecific: {
                sessionVariables: {
                    application_name: "typeorm_session_variables_test",
                },
            },
        })
    })
    ...
    it("should apply configured session variables on pooled connections", () =>
        Promise.all(
            dataSources.map(async (dataSource) => {
                const result = await dataSource.query("SHOW application_name")
                expect(result[0].application_name).to.equal(
                    "typeorm_session_variables_test",
                )
            }),
        ))
})

describe("driver > session variables > invalid configuration", () => {
    it("should fail initialization when a session variable cannot be applied", async () => {
        ...
        const dataSource = new DataSource({
            ...
            sessionVariables: { not_a_real_session_variable: "x" },
