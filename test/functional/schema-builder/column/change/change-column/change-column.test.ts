...
describe("schema builder > change column", () => {
    ...
    it("should alter postgres varchar length without dropping existing data", async () => {
        const postgresDataSources = dataSources.filter(
            (dataSource) => dataSource.driver.options.type === "postgres",
        )

        await Promise.all(
            postgresDataSources.map(async (dataSource) => {
                const queryRunner = dataSource.createQueryRunner()

                await queryRunner.createTable(
                    new Table({
                        name: "change_column_varchar_length",
                        columns: [
                            {
                                name: "id",
                                type: "int",
                ...
                await queryRunner.query(
                    `INSERT INTO "change_column_varchar_length"("id", "example") VALUES (1, 'value')`,
                )

                const table = await queryRunner.getTable(
                    "change_column_varchar_length",
                )
                ...
                expect(upQueries).to.eql([
                    `ALTER TABLE "change_column_varchar_length" ALTER COLUMN "example" TYPE character varying(51)`,
                ])
                ...
                const rows = await queryRunner.query(
                    `SELECT "example" FROM "change_column_varchar_length" WHERE "id" = 1`,
                )
                expect(rows).to.eql([{ example: "value" }])

                const changedTable = await queryRunner.getTable(
                    "change_column_varchar_length",
                )
                expect(
