...
describe("schema builder > change column", () => {
    ...
    it("should preserve existing postgres data when changing column length", () =>
        Promise.all(
            dataSources.map(async (dataSource) => {
                ...
                upQueries.should.contain(
                    `ALTER TABLE "post" ALTER COLUMN "name" TYPE character varying(500)`,
                )
                upQueries.should.not.contain(`DROP COLUMN "name"`)
                upQueries.should.not.contain(`ADD "name"`)
                ...
                post.name.should.be.equal("kept")
