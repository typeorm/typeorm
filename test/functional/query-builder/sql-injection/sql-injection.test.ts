...
describe("query builder > sql injection", () => {
    ...
    describe("distinctOn", () => {
        it("should escape valid distinct on property paths", () => {
            for (const dataSource of dataSources) {
                ...
                const sql = dataSource
                    .createQueryBuilder(Post, "post")
                    .distinctOn(["post.name", "post.id"])
                    .getSql()

                expect(sql).to.contain(
                    'DISTINCT ON ("post"."name", "post"."id")',
                )
            }
        })

        it("should escape distinct on values for aliases without metadata", () => {
            for (const dataSource of dataSources) {
                if (!DriverUtils.isPostgresFamily(dataSource.driver)) {
                    continue
                }
                ...
                const sql = dataSource
                    .createQueryBuilder()
                    .from("post", "post")
                    .distinctOn(["post.name"])
                    .getSql()

                expect(sql).to.contain('DISTINCT ON ("post"."name")')
            }
        })
        ...
        it("should escape injection payloads that carry a registered alias prefix", () => {
            for (const dataSource of dataSources) {
                ...
                for (const payload of [
                    ...
                    expect(sql).to.contain(
                        `DISTINCT ON ("post"."${payload.slice("post.".length)}")`,
                    )
                }
            }
        })

        it("should escape injection payloads without a registered alias prefix", () => {
            for (const dataSource of dataSources) {
                if (!DriverUtils.isPostgresFamily(dataSource.driver)) {
                    continue
                }
                ...
                const sql = dataSource
                    .createQueryBuilder(Post, "post")
                    .distinctOn(["(SELECT pg_sleep(5))"])
                    .getSql()

                expect(sql).to.contain('DISTINCT ON ("(SELECT pg_sleep(5))")')
                expect(sql).to.not.contain("DISTINCT ON ((SELECT pg_sleep(5)))")
            }
        })
        ...
        it("should not interpolate a raw malicious payload into the generated SQL", async () => {
            for (const dataSource of dataSources) {
                ...
                expect(sql).to.not.contain("DISTINCT ON (post.id) DROP TABLE")
