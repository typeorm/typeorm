...
:::note Identifier quoting
Quoting rules differ between databases: PostgreSQL, CockroachDB, Oracle, and SAP HANA use double quotes (`"user"`), SQL Server uses square brackets (`[user]`), while MySQL, MariaDB, SQLite, and Spanner accept unquoted identifiers for `user`. The examples below intentionally follow each database's native conventions.
:::
...
Spanner has no `EXPLAIN` SQL keyword. Use the [Query Plan visualizer](https://cloud.google.com/spanner/docs/query-execution-plans#query_plan_visualizer) in the Google Cloud Console, or run a query in `PLAN`/`PROFILE` mode via the `gcloud` CLI:
