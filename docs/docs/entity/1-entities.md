...
- `temporal: boolean | { timeZone: string }` - Opt-in flag to hydrate and persist date/time columns as [TC39 Temporal](https://github.com/tc39/proposal-temporal) types instead of `Date`. Maps `date` → `Temporal.PlainDate`, `time` → `Temporal.PlainTime`, `timestamp`/`datetime` → `Temporal.PlainDateTime`, `timestamptz` → `Temporal.ZonedDateTime` (requires `{ timeZone: "..." }`), and `interval` (Postgres) → `Temporal.Duration`. Requires `globalThis.Temporal` (Node 26+ or a Temporal-capable runtime). Supported on PostgreSQL and MySQL/MariaDB; on the Aurora data-api drivers it applies only when `formatOptions.castParameters` is `false`.
