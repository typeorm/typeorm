...
- `temporal: boolean | { timeZone: string }` - Opt-in flag to hydrate/persist date/time columns as [TC39 Temporal](https://github.com/tc39/proposal-temporal) types instead of `Date`. Wall-clock columns (postgres `timestamp`, mysql `datetime`, `date`, `time`) map to `PlainDateTime`/`PlainDate`/`PlainTime`; moment-in-time columns (postgres `timestamptz`, mysql/mariadb `timestamp`) map to `ZonedDateTime` (UTC by default, or `{ timeZone }` for a specific IANA zone); postgres `interval` maps to `Duration`. Requires a Temporal implementation — pass `temporal` in `DataSource` options (e.g. `{ temporal: Temporal }` from `@js-temporal/polyfill`) or use Node 26+.
