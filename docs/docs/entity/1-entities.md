...
- `temporal: boolean | { timeZone: string }` - Opt-in flag to hydrate and persist date/time columns as [TC39 Temporal](https://github.com/tc39/proposal-temporal) types instead of `Date`. `{ timeZone }` is required for `timestamptz` → `Temporal.ZonedDateTime`. Requires `globalThis.Temporal`.
