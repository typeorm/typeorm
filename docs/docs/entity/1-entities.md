...
- `generatedType: "VIRTUAL"|"STORED"` - Generated column type. Supported by PostgreSQL 18+ (`VIRTUAL` supported, `STORED` for older versions), CockroachDB, MySQL/MariaDB, Oracle (`VIRTUAL` only), Spanner (`STORED` only), SQLite, and SQL Server (`STORED` only, internally mapped to `PERSISTED`).
