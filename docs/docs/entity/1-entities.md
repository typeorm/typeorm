...
Vector columns are supported on MariaDB/MySQL, PostgreSQL (via
[`pgvector`](https://github.com/pgvector/pgvector) extension) and SAP HANA
Cloud, enabling storing and querying vector embeddings for similarity search and
...
-   `vector` - stores vectors as 4-byte floats (single precision)
    -   MariaDB/MySQL: built-in `vector` type
