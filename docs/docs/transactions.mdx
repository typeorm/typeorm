...
CockroachDB does not implement weaker isolation levels at all. When you request `READ UNCOMMITTED` or `REPEATABLE READ` via SQL (e.g. `BEGIN TRANSACTION ISOLATION LEVEL READ UNCOMMITTED`), CockroachDB accepts the syntax, silently ignores the requested weakness, and runs the transaction at a stricter level instead — no warning, no error:

- `READ UNCOMMITTED` → runs as `READ COMMITTED`
- `REPEATABLE READ` → runs as `SERIALIZABLE`
...
- `startTransaction(isolationLevel?, savepointName?)` - starts a new transaction inside the query runner instance. When called inside an existing transaction, creates a `SAVEPOINT`. Pass a custom `savepointName` to avoid conflicts when running nested transactions concurrently.
- `commitTransaction(savepointName?)` - commits all changes made using the query runner instance.
- `rollbackTransaction(savepointName?)` - rolls all changes made using the query runner instance back.
