...
MongoDB transactions are supported when your MongoDB deployment supports them, such as a replica set or sharded cluster.

When using MongoDB, you can also pass MongoDB transaction options as the first argument to `DataSource.transaction()` or `MongoEntityManager.transaction()`:

```typescript
await myDataSource.transaction(
    {
...
TypeORM forwards the options to MongoDB's `session.startTransaction()` call, so the supported properties follow the MongoDB driver's `TransactionOptions` type.
