...
- `clear` - Clears all the data from the given table (truncates/drops it). Supports cascading on PostgreSQL and Oracle via `{ cascade: true }` (other drivers ignore the option and an error is thrown when requested on unsupported platforms).

```typescript
await manager.clear(User)

// Postgres / Oracle only: cascade to dependent tables
