...
With this mode, no queries will go to the read replicas by default, and you'll have to opt-in to sending queries to read replicas with explicit `.createQueryRunner("replica")` calls.
