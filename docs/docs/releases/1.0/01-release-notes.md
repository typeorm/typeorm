...
- **Columns with `select: false` no longer returned by read queries** — columns marked with `select: false` are now correctly excluded from `find` and query-builder results; use `addSelect` to opt-in. Note that `select: false` only controls what is loaded from the database on a read — the in-memory entity returned by `repository.save()` (and the reference you passed in) is left untouched, so any value you supplied is still available on it after persistence ([#11944](https://github.com/typeorm/typeorm/pull/11944) by [@gioboa](https://github.com/gioboa), [#12501](https://github.com/typeorm/typeorm/pull/12501) by [@tada5hi](https://github.com/tada5hi))
