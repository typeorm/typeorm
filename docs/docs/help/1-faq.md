...
## How do dates and timezones work in TypeORM?

Timezone handling is a common source of confusion. Here's what you need to know to avoid pitfalls.

### The `utc` column option

For `date`-type columns, TypeORM can automatically convert values to/from UTC:

```typescript
@Column({
    type: "date",
    utc: true, // store and retrieve in UTC instead of local time
})
expiredAt: string;
```

When `utc: true`, values are converted to UTC before being sent to the database and converted back to UTC when read. For `timestamp` columns, use the DataSource `timezone` option instead.

### Configure the DataSource timezone

Set the connection timezone to align with your application:

```typescript
const dataSource = new DataSource({
    // ...
    timezone: "Z",       // use UTC (equivalent to "+00:00")
    // or
    timezone: "+00:00",  // UTC offset
    // or
    timezone: "local",   // use the server's local timezone
});
```

### Align Node.js and database timezones

The most important rule: **your Node.js process and your database server should use the same timezone**. When they differ, you will see dates shifted by the timezone offset.

Set the `TZ` environment variable when launching Node.js:

```bash
# Unix / macOS
TZ=UTC node dist/main.js
...
Or set it programmatically before creating the DataSource:

```typescript
process.env.TZ = "UTC";
```

### Per-database timezone settings
...
Or in the connection string: `postgres://user:pass@host:5432/db?options=-c%20timezone%3DUTC`
...
- **Date-only columns (`type: "date"`)**: If you store `"2025-05-01"` and the Node.js server is in PST, querying may return the wrong day. Use `utc: true` or store as a string (`"YYYY-MM-DD"`).
- **`new Date()` in `default` columns**: The value is generated by Node.js, not the database. Ensure Node.js and DB timezones match, or use database functions like `NOW()` instead.
- **Migrations**: If you modify the `TIMESTAMP` column type, re-run migrations after changing timezone settings.

### Summary checklist

- [ ] Set `timezone: "Z"` (or `utc: true` for `date` columns) in your DataSource
- [ ] Set `TZ=UTC` environment variable for your Node.js process
- [ ] Ensure your database server also runs in UTC
- [ ] When inserting test dates, verify they are stored as UTC in the database

See also: [MSSQL datetime issues](#) and [MySQL timezone mismatches](#) for common error patterns.
