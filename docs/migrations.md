...
If you need to revert multiple migrations you must call this command multiple times, or, you can use `until` argument:
```
typeorm migration:revert --until=MigrationName
```
This will revert all migrations that have run after migration with name `MigrationName`.
