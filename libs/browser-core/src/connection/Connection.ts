...
export class Connection {
    ...
    constructor(options: ConnectionOptions,
                protected connectionMetadataBuilderCls = ConnectionMetadataBuilder,
                driverFactoryCls = DriverFactory,
                queryResultCacheFactoryCls = QueryResultCacheFactory,
                loggerFactoryCls = LoggerFactory) {
        this.name = options.name || "default";
        this.options = options;
        this.logger = new loggerFactoryCls().create(this.options.logger, this.options.logging);
        this.driver = new driverFactoryCls().create(this);
        this.manager = new this.driver.entityManagerCls(this);
        this.namingStrategy = options.namingStrategy || new DefaultNamingStrategy();
        this.queryResultCache = options.cache ? new queryResultCacheFactoryCls(this).create() : undefined;
     ...
     */
    async connect(): Promise<this | Connection> {
        if (this.isConnected)
    ...
    async dropDatabase(): Promise<void> {
        ...
        try {
            if (this.driver.type === "SqlServerDriver" || this.driver.type === "MysqlDriver" || this.driver.type === "AuroraDataApiDriver") {
    ...
    //     if (!(this.driver.type === "MongoDriver"))
    ...
    async query(query: string, parameters?: any[], queryRunner?: QueryRunner): Promise<any> {
        if (this.type === 'MongoEntityManager')
    ...
    createQueryBuilder<Entity>(entityOrRunner?: ObjectType<Entity> | EntitySchema<Entity> | Function | string | QueryRunner, alias?: string, queryRunner?: QueryRunner): SelectQueryBuilder<Entity> {
        ...
        if (alias) {
            ...
            return new this.driver.SelectQueryBuilderCls(this, queryRunner)
                .select(alias)
                .from(metadata.target, alias);

        } else {
            return new this.driver.SelectQueryBuilderCls(this, entityOrRunner as QueryRunner | undefined);
        }
    }
    ...
    createQueryRunner(mode: "master" | "slave" = "master"): QueryRunner {
        ...
        const manager = new this.driver.entityManagerCls(this, queryRunner);
    ...
    protected findMetadata(target: Function | EntitySchema<any> | string): EntityMetadata | undefined {
        return this.entityMetadatas.find(metadata =>
            metadata.target === target
                ? true : target instanceof EntitySchema
    ...
    protected buildMetadatas(): void {
        ...
        const connectionMetadataBuilder = new this.connectionMetadataBuilderCls(this);
