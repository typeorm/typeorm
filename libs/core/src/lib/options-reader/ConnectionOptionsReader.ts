...
import { ConnectionOptionsEnvReader } from "./ConnectionOptionsEnvReader";
import { ConnectionOptionsYmlReader } from "./ConnectionOptionsYmlReader";
import { ConnectionOptionsXmlReader } from "./ConnectionOptionsXmlReader";
...
export class ConnectionOptionsReader {
    ...
    protected async load(): Promise<ConnectionOptions[] | undefined> {
        ...
        const foundFileFormat = fileExtension || fileFormats.find(format => {
            return existsSync(this.baseFilePath + "." + format);
        });

        // if .env file found then load all its variables into process.env using dotenv package
        if (foundFileFormat === "env") {
            config({path: this.baseFilePath});
        ...
        if (process.env["TYPEORM_CONNECTION"] || process.env["TYPEORM_URL"]) {
