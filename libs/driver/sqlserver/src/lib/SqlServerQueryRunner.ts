...
export class SqlServerQueryRunner extends BaseQueryRunner implements QueryRunner {
    ...
    async query(query: string, parameters?: any[]): Promise<any> {
        ...
        const promise = new Promise(async (ok, fail) => {
            try {
                ...
                const pool = await (this.mode === "slave" ? this.driver.obtainSlaveConnection() : this.driver.obtainMasterConnection());
                const request = new Request(this.isTransactionActive ? this.databaseConnection : pool);
                if (parameters && parameters.length) {
                    parameters.forEach((parameter, index) => {
                        if (parameter instanceof MssqlParameter) {
                            const mssqlParameter = this.mssqlParameterToNativeParameter(parameter);
                            if (mssqlParameter) {
                                request.input(index.toString(), mssqlParameter, parameter.value);
                            } else {
                                request.input(index.toString(), parameter.value);
                            }
                        } else {
                            request.input(index.toString(), parameter);
                        }
                    });
                }
                ...
                request.query(query, (err: any, result: any) => {
                    ...
                    const promiseIndex = this.queryResponsibilityChain.indexOf(promise);
                    const waitingPromiseIndex = this.queryResponsibilityChain.indexOf(waitingPromise);
                    if (err) {
     ...
     */
    async stream(query: string, parameters?: any[], onEnd?: (...args: any[]) => void, onError?: (...args: any[]) => void): Promise<ReadStream> {
        if (this.isReleased)
            throw new QueryRunnerAlreadyReleasedError();
        ...
        const promise = new Promise<ReadStream>(async (ok, fail) => {
            ...
            if (parameters && parameters.length) {
                parameters.forEach((parameter, index) => {
                    if (parameter instanceof MssqlParameter) {
                        request.input(index.toString(), this.mssqlParameterToNativeParameter(parameter), parameter.value);
                    } else {
                        request.input(index.toString(), parameter);
                    }
                });
    ...
    protected mssqlParameterToNativeParameter(parameter: MssqlParameter): any {
        switch (this.driver.normalizeType({type: parameter.type as any})) {
            ...
            case "bigint":
                return BigInt;
            case "decimal":
                return Decimal(...parameter.params);
            case "float":
                return Float;
            case "int":
                return Int;
            case "money":
                return Money;
            case "numeric":
                return Numeric(...parameter.params);
            case "smallint":
                return SmallInt;
            case "smallmoney":
                return SmallMoney;
            ...
            case "tinyint":
                return TinyInt;
            case "char":
                return Char(...parameter.params);
            case "nchar":
                return NChar(...parameter.params);
            case "text":
                return Text;
            case "ntext":
                return NText;
            case "varchar":
                return VarChar(...parameter.params);
            case "nvarchar":
                return NVarChar(...parameter.params);
            case "xml":
                return Xml;
            case "time":
                return Time(...parameter.params);
            case "date":
                return mssql.Date;
            case "datetime":
                return DateTime;
            case "datetime2":
                return DateTime2(...parameter.params);
            case "datetimeoffset":
                return DateTimeOffset(...parameter.params);
            case "smalldatetime":
            ...
            case "binary":
                return Binary;
            case "varbinary":
                return VarBinary(...parameter.params);
            case "image":
                return Image;
            case "udt":
                return UDT;
