...
export class SqljsDriver extends AbstractSqliteDriver {
    ...
    /**
     * Loads a database from a local storage key or array.
    ...
    async load(fileNameOrLocalStorageOrData: string | Uint8Array, checkIfFileOrLocalStorageExists: boolean = true): Promise<any> {
        if (typeof fileNameOrLocalStorageOrData === "string") {
            ...
            if (this.options.useLocalForage) {
                ...
                localStorageContent = localStorage.getItem(fileNameOrLocalStorageOrData);
            }
    ...
    /**
     * Saved the current database to the given local storage key or
    ...
    async save(location?: string) {
        ...
        if (this.options.useLocalForage) {
            ...
            localStorage.setItem(path, JSON.stringify(databaseArray));
        }
    }
     ...
     * otherwise the save method is called which saves it to local storage
