...
import { BLOB, CLOB, createPool, DATE, NUMBER, STRING } from 'oracledb';
import { AbstractSqlDriver } from '@typeorm/driver-sql-abstract';

/**
 * Organizes communication with Oracle RDBMS.
 */
export class OracleDriver extends AbstractSqlDriver implements Driver {
    ...
    protected async createPool(options: OracleConnectionOptions, credentials: OracleConnectionCredentialsOptions): Promise<any> {
        ...
        return new Promise<void>((ok, fail) => {
            createPool(connectionOptions, (err: any, pool: any) => {
                if (err)
                    return fail(err);
                ok(pool);
            });
