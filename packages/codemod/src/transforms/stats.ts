    ...
    collect: {
        ...
        todos: (raw: Record<string, number>): Map<string, string[]> => {
            ...
            for (const key of Object.keys(raw)) {
                ...
                // Keys are always `todo:<transform>:<path>` — the first colon
                // separates the transform name from the file path. Skip
                // malformed keys (no colon, empty transform) rather than
                // producing garbled output via `slice(0, -1)`. File paths on
                // Windows contain `:` (e.g. `C:\...`) so we only split on the
