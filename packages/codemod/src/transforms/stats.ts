    ...
    collect: {
        ...
        todos: (raw: Record<string, number>): Map<string, string[]> => {
            ...
            for (const key of Object.keys(raw)) {
                ...
                // Keys emitted by transforms follow the shape
                // `<prefix><transform>:<path>` (see `TODO_PREFIX`). The first
                // colon in `rest` separates the transform name from the file
                // path. Skip malformed keys (no colon, empty transform)
                // rather than producing garbled output via `slice(0, -1)`.
                // File paths on Windows contain `:` (e.g. `C:\...`) so we
                // only split on the FIRST colon, preserving the rest verbatim.
