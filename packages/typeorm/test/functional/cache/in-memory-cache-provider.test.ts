...
import { InMemoryResultCache } from "../../../src/cache/InMemoryResultCache"
    ...
    it("should be used instead of built-ins", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                expect(queryResultCache).to.have.property("inMemoryCache")

                const cache = queryResultCache.inMemoryCache
                expect(cache).to.be.instanceof(LRUCache)
            }),
        ))
    ...
    it("should cache results properly", () =>
        Promise.all(
            connections.map(async (connection) => {
                ...
                // without cache it must return really how many entities there are
