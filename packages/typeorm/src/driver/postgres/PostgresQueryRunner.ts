    ...
    implements QueryRunner
{
    ...
    protected buildCreateColumnSql(table: Table, column: TableColumn) {
        ...
        // Postgres supports both STORED and VIRTUAL generated columns (VIRTUAL since PostgreSQL 18)
        if (
            (column.generatedType === "STORED" ||
                column.generatedType === "VIRTUAL") &&
            column.asExpression
        ) {
            const generatedType = column.generatedType === "VIRTUAL" ? "VIRTUAL" : "STORED"
            c += ` GENERATED ALWAYS AS (${column.asExpression}) ${generatedType}`
        }
